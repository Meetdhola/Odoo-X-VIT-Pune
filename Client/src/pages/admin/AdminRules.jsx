...
const AdminRules = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.error('Failed to eliminate rule');
    }
  };
  ...
  return (
    <AdminLayout title="Approval Rules" subtitle="Ways to approve expenses">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pb-20">
          {loading ? (
            ...
            <div className="col-span-2 glass-card rounded-[2.5rem] border-white/5 p-20 text-center bg-white/[0.01]">
              ...
              <p className="text-xs text-slate-600 font-black uppercase tracking-[0.3em] mb-2 italic">No Rules Yet</p>
              <p className="text-[10px] text-slate-700 font-bold uppercase tracking-widest">Create an approval rule to manage how expenses are handled.</p>
            </div>
          ) : rules.map((rule) => (
            <div key={rule._id} className="glass-card rounded-[2.5rem] border-white/5 p-8 shadow-2xl bg-white/[0.01] hover:bg-white/[0.02] hover:border-indigo-500/30 transition-all group relative overflow-hidden">
              ...
              <div className="pt-6 border-t border-white/5 flex items-center justify-between relative z-10">
                <p className="text-[10px] text-slate-600 font-bold uppercase tracking-widest italic flex items-center gap-2">
                  ...
                  {rule.conditionType === 'sequential' && "Everyone must approve in order"}
                  {rule.conditionType === 'percentage' && `Majority needs to approve: ${rule.percentageThreshold}% Threshold`}
                  {rule.conditionType === 'specific' && `Only one person approves: ${rule.specificApprover?.name}`}
                  {rule.conditionType === 'hybrid' && `Percentage OR specific person required`}
                </p>
                <div className="flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
                  <span className="text-[9px] text-slate-700 font-black uppercase tracking-widest">On</span>
                </div>
              </div>
            </div>
          ))}
                ...
                <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-10 space-y-10 scrollbar-hide bg-gradient-to-b from-slate-900 to-slate-950">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {[
                        { id: 'sequential', label: 'One-by-One', desc: 'Step-by-step', icon: ArrowRight },
                        { id: 'percentage', label: 'Consensus', desc: 'Percent threshold', icon: Percent },
                        { id: 'specific', label: 'Single Person', desc: 'One person decides', icon: UserCheck },
                        { id: 'hybrid', label: 'Mixed', desc: 'Group or specific person', icon: GitMerge }
                      ].map(type => (
                        <div
                          key={type.id}
                          ...
                          <div key={idx} className="flex items-center gap-4 bg-white/[0.02] p-3 rounded-2xl border border-white/5 group transition-all hover:bg-white/[0.04]">
                            ...
                            {/*
                                 <select 
                                   ...
                                   required
                                 >
                                   <option value="" className="bg-slate-900">Choose person...</option>
...

                  {
                    (formData.conditionType === 'percentage' || formData.conditionType === 'hybrid') && (
                      <div className="p-8 bg-indigo-500/5 border border-indigo-500/10 rounded-[2.5rem] space-y-4">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] italic">Approval Threshold (%)</label>
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-[9px] text-slate-600 font-bold uppercase tracking-widest mt-0.5 whitespace-nowrap">* How many people need to say yes</p>
                          </div>
                        </div>
                      </div>
                    )
                    ...
                    (formData.conditionType === 'specific' || formData.conditionType === 'hybrid') && (
                      <div className="space-y-4">
                        ...
                        <select
                          ...
                          required
                        >
                          <option value="" className="bg-slate-900 italic">Choose specific approver...</option>
                          {users.filter(u => u.role !== 'Employee').map(u => <option key={u._id} value={u._id} className="bg-slate-900">{u.name} ({u.role})</option>)}
                        </select>
                      </div>
                    )
